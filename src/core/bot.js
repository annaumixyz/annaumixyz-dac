...
const CONFIG_DIR = path.join(process.env.XDG_CONFIG_HOME || path.join(os.homedir(), '.config'), 'annaumixyz-dac');
...
class DACBot {
  ...
  // âââ Step Tracking ââââââââââââââââââââââââââââââââââââ
  ...
  persistSession(cookieString, csrf) {
    ...
    // If no account name, still don't overwrite the whole config file â write to a sidecar session file instead
  ...
  async sendNative(to, amountEth) {
    ...
    }, { detail: `${shortAddr(this.wallet.address)} â ${shortAddr(to)} ${amountEth} DACC`, txMeta: true });
  }
  ...
  async burnForQE(amountEth) {
    ...
    }, { detail: `${amountEth} DACC â QE`, txMeta: true });
  }
